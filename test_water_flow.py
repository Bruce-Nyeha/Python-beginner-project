...
def test_reynolds_number():
    assert reynolds_number(0.048692, 0.00) == approx(0, abs=10)
    assert reynolds_number(0.048692, 1.65) == approx(80069, abs=10)
    assert reynolds_number(0.048692, 1.75) == approx(84922, abs=10)
    assert reynolds_number(0.286870, 1.65) == approx(471729, abs=10)
    assert reynolds_number(0.286870, 1.75) == approx(500318, abs=10)

# Pressure Loss from Pipe Reduction
def test_pressure_loss_from_pipe_reduction():
    assert pressure_loss_from_pipe_reduction(0.28687, 0.00, 10, 0.048692) == approx(0.000, abs=0.001)
