# waterflow.py
# Fixed by Ghana's Top Engineer | November 10, 2025
...
WATER_DENSITY = 998.2                  # ← FIXED: space after =
WATER_DYNAMIC_VISCOSITY = 0.0010016     # ← ADD THIS!
...
def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):
    ...
    denominator = 2000 * pipe_diameter    # ← FIXED: 2000 not 200
    return numerator / denominator

def pressure_loss_from_fittings(fluid_velocity, quantity_fittings):
    return -0.04 * WATER_DENSITY * (fluid_velocity ** 2) * quantity_fittings / 2000  # ← **2 fixed

def reynolds_number(hydraulic_diameter, fluid_velocity):
    return (WATER_DENSITY * hydraulic_diameter * fluid_velocity) / WATER_DYNAMIC_VISCOSITY  # ← FIXED

def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    k = (0.1 + 50 / reynolds_number) * (((larger_diameter / smaller_diameter) ** 4) - 1)  # ← -1 fixed
